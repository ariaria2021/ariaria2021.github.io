...
今回の移植で、ロジックの変更は **0行** でした。主な変更点は以下の3つだけです。

- **Hooks**: `react` から `preact/hooks` へ。
- **memo / compat**: `react` から `preact/compat` へ。
- **エントリーポイント**: `createRoot` から Preact の `render` へ。

コードベースの9割以上をそのまま流用でき、開発体験（DX）を損なうことなく軽量化に成功しました。
